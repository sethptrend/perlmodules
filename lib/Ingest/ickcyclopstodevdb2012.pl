...
my $dbtest = Connection::StarfishTest->new();
...
$dbtest->DoSQL("Set IDENTITY_INSERT networkdbor.dbo.icks ON");#we're going to be inserting identities, hopefully this works for the whole session
...
for my $row (@$table){
	...
	$dbtest->InsertValues('networkdbor.dbo.ports', %a_port);
	...
	$dbtest->InsertValues('networkdbor.dbo.ports', %z_port);
	...
	$dbtest->InsertValues('networkdbor.dbo.icks', %ick);
#	print "\n";
